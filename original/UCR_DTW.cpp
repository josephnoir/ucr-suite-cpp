...
/// siginifant benefits. And using the first and last points can be computed in
/// constant time. The prunning power of LB_Kim is non-trivial, especially when
/// the query is not long, say in length 128.
...
/// in DTW. l,u: lower and upper envelop of the current data
...
int main(int argc, char* argv[]) {
  ...
  int it = 0, ep = 0, k = 0;
  long long
      I;  /// the starting index of the data in current chunk of size EPOCH
  ...
  while (!done) {
    ...
    if (ep <= m - 1) {
      ...
      /// Just for printing a dot for approximate a million point. Not much
      /// accurate.
      ...
      for (i = 0; i < ep; i++) {
        ...
        if (i >= m - 1) {
          ...
          if (lb_kim < bsf) {
            ...
            /// will be computed on the fly. uo, lo are envelop of the query.
            lb_k = lb_keogh_cumulative(order, t, uo, lo, cb1, j, m, mean, std,
                                       bsf);
            if (lb_k < bsf) {
              ...
              lb_k2 = lb_keogh_data_cumulative(order, tz, qo, cb2, l_buff + I,
                                               u_buff + I, m, mean, std, bsf);
              if (lb_k2 < bsf) {
                /// Choose better lower bound between lb_keogh and lb_keogh2 to
                /// be used in early abandoning DTW Note that cb and cb2 will be
                /// cumulative summed here.
                if (lb_k > lb_k2) {
                  cb[m - 1] = cb1[m - 1];
                  for (k = m - 2; k >= 0; k--)
                    cb[k] = cb[k + 1] + cb1[k];
  ...
  cout << "Total Execution Time : " << (t2 - t1) / CLOCKS_PER_SEC << " sec"
       << endl;
